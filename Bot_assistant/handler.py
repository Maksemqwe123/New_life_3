...
img_files_jokes = r'C:/Users/makce/PycharmProjects/pythonProject/New_life_3/Bot_assistant/img'

img_jokes = os.listdir(img_files_jokes)
...
async def send_all(message: types.Message):
    if message.chat.type == 'private':
        if message.from_user.id == ADMIN_ID:
            text = message.text[9:]
            users_id = database.select_all_users()
            for row in users_id:
                try:
                    await bot.send_message(row[0], text)

                except BotBlocked:
                    users_blocked = database.select_all_users()
                    for blocked_bot in users_blocked:
                        database.delete_user_main_table(blocked_bot[0])
...
async def today(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        if 10 < cur_weather < 5:
            ...
            await bot.send_photo(chat_id=message.from_user.id, photo=open(f'{img_files_jokes}/' + random.choice(img_jokes), 'rb'))
...
async def street(message: types.Message):
    ...
    await message.answer('В каком городе вы хотите посмотреть развлечения🎬☕🍕', reply_markup=user_street_kb)
...
async def cinema(message: types.Message):
    ParserGenre(location_city_name)
