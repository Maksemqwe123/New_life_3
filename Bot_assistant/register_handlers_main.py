...
def register_handlers_weather_house_street(dp: Dispatcher):
    dp.register_message_handler(today, content_types=["location"], state=WeatherToday.weather_today_city)
    dp.register_message_handler(today, Text(equals=cities, ignore_case=True), state=WeatherToday.weather_today_city)
