...
class Database:
    ...
    def set_active(self, active, user_id):
        with self.connection:
            return self.cursor.execute("UPDATE users_bot SET active = ? WHERE user_id = ?", (active, user_id, ))
    ...
    def delete_users(self):
        with self.connection:
            return self.cursor.execute("""DELETE FROM users_bot""")


db = Database(r'/New_life_3/database')

user = db.all_user_db()
user_active = db.set_active(1908084892, 0)
print(user_active)
print(user)
